...
class Trainer():
    ...
    def _train_epoch(self, epoch):
        ...
        pbar.close()
        return mean(rec_all_loss_list),  mean(user_gan_all_loss_list), mean(auc_list)
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_GAN_MRNN = self.args.checkpoint_dir + ('checkpoint-Zeroshot_GAN_MRNN-epoch{}.pth'.format(epoch))
        torch.save(state_model, filename_GAN_MRNN)

    def train(self):
        for epoch in range(1, self.args.epoch+1):
            rec_loss, gan_loss_user, rec_auc = self._train_epoch(epoch)
    ...
    def test(self):
        ...
        predict_df.to_csv('Zeroshot_GAN_MRNN_predict.csv', index = False)
