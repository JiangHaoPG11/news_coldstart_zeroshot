...
class Trainer():
    def __init__(self, args, LightGCN_VAE_model, optimizer_LightGCN_VAE, data):
        self.args = args
        self.LightGCN_VAE_model = LightGCN_VAE_model
        self.optimizer_LightGCN_VAE = optimizer_LightGCN_VAE
        self.save_period = 1
        self.vaild_period = 1
    ...
    def optimize_model(self, loss):
        self.optimizer_LightGCN_VAE.zero_grad()
        loss.backward()
        #for name, param in self.model_recommender.named_parameters():
        #    print('%14s : %s' % (name, param.grad))
        self.optimizer_LightGCN_VAE.step()
        return loss

    def _train_epoch(self, epoch):
        self.LightGCN_VAE_model.train()
        rec_all_loss_list = []
        auc_list = []
        rec_all_loss = 0
        ...
        for data in self.train_dataloader:
            ...
            loss, rec_score, _, _ = self.LightGCN_VAE_model(user_index, candidate_newsindex, user_clicked_newsindex, label)
    ...
    def _vaild_epoch(self):
        self.LightGCN_VAE_model.eval()
        rec_auc_list = []
        with no_grad():
            pbar = tqdm(total=self.vailddata_size)
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.LightGCN_VAE_model.test(user_index, candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_LightGCN_VAE_model = self.LightGCN_VAE_model.state_dict()
        filename_LightGCN_VAE = self.args.checkpoint_dir + ('checkpoint-LightGCN_VAE-epoch{}.pth'.format(epoch))
        torch.save(state_LightGCN_VAE_model, filename_LightGCN_VAE)
    ...
    def test(self):
        ...
        with no_grad():
            ...
            pbar.close()
        # 存储预测结果
        folder_path = '../predict/LightGCN_VAE/'
        ...
        predict_df.to_csv('LightGCN_VAE_predict.csv', index = False)
