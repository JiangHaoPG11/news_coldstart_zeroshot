...
class Trainer():
    def __init__(self, args, exp1_model,  optimizer_base, optimizer_Zeroshot_news, optimizer_Zeroshot_user, data):
        self.args = args
        self.exp1_model = exp1_model
    ...
    def optimize_model(self, rec_score, kl_loss, label):
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score, loss_zeroshot_news, loss_zeroshot_user, kl_loss = self.exp1_model(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
            # rec_score, kl_loss = self.exp1_model(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
            # news_zeroshot_all_loss_list.append(1)
            # user_zeroshot_all_loss_list.append(1)

            rec_loss = self.optimize_model(rec_score, kl_loss, label.to(self.device))
            ...
            news_zeroshot_all_loss_list.append(torch.mean(loss_zeroshot_news).cpu().item())
        ...
        pbar.close()
        return mean(rec_all_loss_list), mean(news_zeroshot_all_loss_list), mean(user_zeroshot_all_loss_list), mean(auc_list)

    def _vaild_epoch(self):
        self.exp1_model.eval()
        rec_auc_list = []
        with no_grad():
            pbar = tqdm(total=self.vailddata_size)
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.exp1_model.test(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_exp1_model = self.exp1_model.state_dict()
        filename_exp1 = self.args.checkpoint_dir + ('checkpoint-Zeroshot_IDvae-epoch{}.pth'.format(epoch))
        torch.save(state_exp1_model, filename_exp1)

    def train(self):
        for epoch in range(1, self.args.epoch+1):
            rec_loss, zeroshot_loss_news, zeroshot_loss_user, rec_auc = self._train_epoch(epoch)
            print("epoch：{}----recommend loss：{}-----zeroshot_news_loss：{}---zeroshot_user_loss：{}---rec auc：{}-------".
                  format(epoch, str(rec_loss), str(zeroshot_loss_news), str(zeroshot_loss_user), str(rec_auc)))
    ...
    def test(self):
        ...
        predict_df.to_csv('Zeroshot_IDvae_predict.csv', index = False)
