...
class Trainer():
    ...
    def optimize_model(self, rec_score, user_gan_loss, label):
        rec_loss = self.criterion(rec_score, torch.argmax(label, dim=1).to(self.device))
        loss = rec_loss + (user_gan_loss) * 0.5
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score, user_gan_loss = self.model(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
            rec_loss = self.optimize_model(rec_score, user_gan_loss, label.to(self.device))
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_GAN_NAML = self.args.checkpoint_dir + ('checkpoint-Zeroshot_GAN_NAML-epoch{}.pth'.format(epoch))
        torch.save(state_model, filename_GAN_NAML)

    def train(self):
        for epoch in range(1, self.args.epoch+1):
            rec_loss, zeroshot_loss_news, zeroshot_loss_user, gan_loss_news, gan_loss_user, rec_auc = self._train_epoch(epoch)
            print("epoch：{}----recommend loss：{}------gan_user_loss：{}--rec auc：{}-------".
                   format(epoch, str(rec_loss), str(gan_loss_user), str(rec_auc)))
    ...
    def test(self):
        ...
        predict_df.to_csv('Zeroshot_GAN_NAML_predict.csv', index = False)
