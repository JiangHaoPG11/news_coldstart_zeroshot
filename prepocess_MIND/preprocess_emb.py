...
def get_word_embedding(model):
    ...
    :return:
    '''
    word_index_df = pd.read_csv('../Data_MIND/df/word_index_df.csv')
    ...
    np.save('../Data_MIND/metadata/news_title_word_embedding.npy', word_embedding)
...
def get_category_embedding(model):
    ...
    :return:
    '''
    category_index_df = pd.read_csv('../Data_MIND/category_index_df.csv')
    ...
    np.save('../Data_MIND/metadata/news_category_embedding.npy', category_embedding)
...
def get_subcategory_embedding(model):
    ...
    :return:
    '''
    subcategory_index_df = pd.read_csv('../Data_MIND/df/subcategory_index_df.csv')
    subcategory_size = subcategory_index_df.shape[0]
    subcategory_list = subcategory_index_df['subcategory'].values.tolist()
    # model = gensim.models.KeyedVectors.load_word2vec_format('../data2/Googlenewss-vectors-negative300.bin',binary=True)
    vector_empty = [0] * 300
    ...
    np.save('../Data_MIND/metadata/news_subcategory_embedding.npy', subcategory_embedding)
