...
def build_KG_network(news_entity_dict, user_clicked_newsindex):
    ...
    graph = pd.read_csv('../Data_MIND/KG/graph_index.csv')
...
def kprn_get_paths(args, kg_env, user_index, candidate_news):
    ...
    np.save('../Data_MIND/pathdata/kprn_path_index.npy', np.array(total_paths_index))
    np.save('../Data_MIND/pathdata/kprn_relations_index.npy', np.array(total_relations_index))
    np.save('../Data_MIND/pathdata/kprn_type_index.npy', np.array(total_type_index))
...
def adac_get_paths(args, kg_env, user_index, ):
    ...
    for m in range(user_clicked_newsindex.shape[1]):
        ...
        total_relations_index.append(relations_index)
        total_type_index.append(type_index)
    np.save('../Data_MIND/pathdata/adac_path_index.npy', np.array(total_paths_index))
    np.save('../Data_MIND/pathdata/adac_relations_index.npy', np.array(total_relations_index))
    np.save('../Data_MIND/pathdata/adac_type_index.npy', np.array(total_type_index))
...
if __name__ == "__main__":
    news_entity_index = np.load('../Data_MIND/metadata/news_entity_index.npy')
    user_index = np.load('../Data_MIND/metadata/user_index.npy')
    candidate_news = np.load('../Data_MIND/metadata/candidate_newsindex.npy')
    user_clicked_newsindex = np.load('../Data_MIND/metadata/user_clicked_newsindex.npy')
    args = parse_args()
