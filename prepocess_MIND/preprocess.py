...
def cal_user_clicked_news_ratio(user_dict, user_clicked_newid_list, new_dict):
    ...
    user_clicked_news_index = []
    for i in range(len(user_clicked_newid_list)):
        if len(user_clicked_newid_list[i]) > 20:
            user_indictor.append('w')
        elif len(user_clicked_newid_list[i]) < 5:
            user_indictor.append('c')
        else:
    ...
    print('正常用户数量{}'.format(cold_user_num))
...
def cal_news_clicked_user_ratio(user_pnewsid_nnews_id_df, user_clicked_df, candidate_newsid_list, news_dict):
    ...
    for i in range(len(pnews_list)):
        pnews_list_one = pnews_list[i]
        nnews_list_one = nnews_list[i]
        for news in pnews_list_one + nnews_list_one:
            if news_dict[news] in news_exposure_dict.keys():
                news_exposure_dict[news_dict[news]].append(user_index_list[i])

    news_exposure_num_list = []
    for key, value in news_exposure_dict.items():
        news_exposure_num_list.append(len(value))
    
    news_exposure_num_list_sort = sorted(list(set(news_exposure_num_list)))

    print(news_exposure_num_list_sort[-1])
    # margin = news_click_num_list_sort[int(len(news_click_num_list_sort) * 0.5)]
    margin = 0
    ...
    plt.xticks(fontsize=10)
    plt.yticks(fontsize=10)
    plt.xlabel("Exposure num", fontsize=16)  # X轴标签
    plt.ylabel('Density', fontsize=16)  # Y轴标签
    plt.title("News exposure distribution", fontsize=18)  # 标题
    plt.grid()  # 添加网格
    plt.show()
    ...
    #     if i not in news_exposure_dict.keys():
    #         news_exposure_dict[i] = 
