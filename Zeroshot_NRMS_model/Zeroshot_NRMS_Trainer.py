...
class Trainer():
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_NRMS = self.args.checkpoint_dir + ('checkpoint-Zeroshot_NRMS-epoch{}.pth'.format(epoch))
        torch.save(state_model, filename_NRMS)
        ...
        for epoch in range(1, self.args.epoch+1):
            ...
            print("epoch：{}----recommend loss：{}-----zeroshot_news_loss：{}---zeroshot_user_loss：{}----la：{}--lc：{}--ld：{}---rec auc：{}-------".
                  format(epoch, str(rec_loss), str(zeroshot_loss_news), str(zeroshot_loss_user),
                         str(La), str(Lc), str(Ld), str(rec_auc)))
    ...
    def test(self):
        ...
        predict_df.to_csv('Zeroshot_NRMS_predict.csv', index = False)
