...
class Trainer():
    def __init__(self, args, base_model,  optimizer_base, data):
        self.args = args
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score = self.base_model(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.base_model.test(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_base_model = self.base_model.state_dict()
        filename_base = self.args.checkpoint_dir + ('checkpoint-Zeroshot_base-epoch{}.pth'.format(epoch))
        torch.save(state_base_model, filename_base)
    ...
    def test(self):
        ...
        predict_df.to_csv('Zeroshot_base_predict.csv', index = False)
