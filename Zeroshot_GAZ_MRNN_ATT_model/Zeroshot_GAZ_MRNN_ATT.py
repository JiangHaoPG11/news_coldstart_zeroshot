...
class news_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, entity_embedding, category_index, subcategory_index):
        ...
        entity_inter = self.norm2(entity_inter)
...
class Zeroshot_GAZ_MRNN_ATT(torch.nn.Module):
    def __init__(self, args, entity_embedding, relation_embedding, news_entity_dict, entity_adj, relation_adj,
        ...
        super(Zeroshot_GAZ_MRNN_ATT, self).__init__()
        self.args = args
        self.device = device
        ...
        # zeroshot学习
        self.zeroshot_news_tower = zeroshot_news_gaz_tower(args)
        self.zeroshot_user_tower = zeroshot_user_gaz_tower(args)
    ...
    def forward(self, candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index):
        ...
        loss_zeroshot_user, user_rep_update = self.zeroshot_user_tower(user_rep, user_att_rep.squeeze(), user_type_index.to(self.device))
        ...
        return rec_score, loss_zeroshot_news, loss_zeroshot_user
    ...
    def test(self, candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index ):
        ...
        # zeroshot学习(Att)
        news_att_rep, user_att_rep = self.get_user_news_att_rep(candidate_newsindex, user_index, user_clicked_newsindex)
        _, user_rep_update= self.zeroshot_user_tower(user_rep, user_att_rep.squeeze(), user_type_index.to(self.device))
