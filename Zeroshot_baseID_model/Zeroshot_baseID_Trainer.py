...
class Trainer():
    def __init__(self, args, baseID_model,  optimizer_base, optimizer_Zeroshot_news, optimizer_Zeroshot_user, data):
        self.args = args
        self.baseID_model = baseID_model
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score, loss_zeroshot_news, loss_zeroshot_user = self.baseID_model(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
    ...
    def _vaild_epoch(self):
        self.baseID_model.eval()
        rec_auc_list = []
        with no_grad():
            pbar = tqdm(total=self.vailddata_size)
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.baseID_model.test(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        state_baseID_model = self.baseID_model.state_dict()
        filename_baseID = self.args.checkpoint_dir + ('checkpoint-Zeroshot_baseID-epoch{}.pth'.format(epoch))
        torch.save(state_baseID_model, filename_baseID)
    ...
    def test(self):
        ...
        predict_df.to_csv('Zeroshot_baseID_predict.csv', index = False)
