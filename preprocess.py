...
def load_data_behaviors_news():
    ...
    '''
    user_behaviors_df = pd.read_table('../Data_MIND/MIND_small/MINDsmall_train/behaviors.tsv',
                                      header=None,
                                      names=['impression_id', 'user_id', 'time', 'history', 'impressions'],
                                      sep="\t")
    news_Info_df = pd.read_table('../Data_MIND/MIND_small/MINDsmall_train/news.tsv',
                                header=None,
                                names=['id', 'category', 'subcategory', 'title', 'abstract', 'url',
                                       'title_entities', 'abstract_entities'])
    entity_embedding_df = pd.read_table('../Data_MIND/MIND_small/MINDsmall_train/entity_embedding.vec', header=None)
...
def get_user_index( user_behaviors_df, user_pnewsid_nnews_id_df):
    ...
    # print(news_index_df)
    # user_index_df.to_csv('../Data_MIND/df/user_index_df.csv', index=False)
    ...
    user_pnewsid_nnews_id_df.to_csv('../Data_MIND/df/user_pnewsid_nnews_id_df.csv')
...
# 计算冷热用户
def cal_user_clicked_news_ratio(user_dict, user_clicked_newid_list, new_dict):
    # # 获取冷热用户：
    # user_indictor = []
    ...
    user_clicked_news_num_list_sort = list(set(sorted(user_clicked_news_num_list)))
    ...
    plt.xlabel("Clicks num", fontsize=16)  # X轴标签
    #plt.ylabel('Density', fontsize=16)  # Y轴标签
    plt.title("User clicks distribution", fontsize=18)  # 标题
    plt.grid()  # 添加网格
    plt.show()
    plt.savefig("User_cold_stastic.png")
    plt.close()

    # margin = user_clicked_news_num_list_sort[int(len(user_clicked_news_num_list_sort) * 0.3)]
    # print('冷用户最大点击数：{}'.format(margin))
    #
    # for i in range(len(user_clicked_newid_list)):
    #     if len(user_clicked_newid_list[i]) > margin:
    ...
    for type in user_indictor:
        if type == 'c':
            ...
            user_type.append(1)
            warm_user_num += 1
    np.save('../Data_MIND/metadata/user_type', np.array(user_type))
    print('热用户数量{}'.format(warm_user_num))
    print('冷用户数量{}'.format(cold_user_num))


### 计算新闻点击率
def cal_news_clicked_user_ratio(user_clicked_df, candidate_newsid_list, news_dict):
    print(len(candidate_newsid_list))
    print(len(news_dict))
    user_clicked_news_list = user_clicked_df['user_clicked_newsid']
    user_index_list = user_clicked_df['user_index']
    # 初始化
    news_user_click_dict = {}
    for i in range(len(candidate_newsid_list)):
        news_user_click_dict[news_dict[candidate_newsid_list[i]]] = []

    for i in range(len(user_clicked_news_list)):
        user_clicked_news = user_clicked_news_list[i]
        for user_clicked_new in user_clicked_news:
            if news_dict[user_clicked_new] in news_user_click_dict.keys():
                news_user_click_dict[news_dict[user_clicked_new]].append(user_index_list[i])

    news_click_num_list = []
    for key, value in news_user_click_dict.items():
        news_click_num_list.append(len(value))
    news_click_num_list_sort = sorted(news_click_num_list)
    margin = news_click_num_list_sort[int(len(news_click_num_list_sort) * 0.3)]
    news_cold_size = int(len(news_click_num_list) * 0.5)
    print('冷新闻最大点击数：{}'.format(margin))

    # 画图
    c1 = sns.color_palette('Set1', 1)
    sns.kdeplot(news_click_num_list, shade=True, color = c1,label = 'news clicked nums')
    # plt.legend(loc='best', fontsize=10)  # 让图例生效
    plt.xticks(fontsize=10)
    plt.yticks(fontsize=10)
    plt.xlabel("Clicks num", fontsize=16)  # X轴标签
    plt.ylabel('Density', fontsize=16)  # Y轴标签
    plt.title("News clicks distribution", fontsize=18)  # 标题
    plt.grid()  # 添加网格
    plt.show()
    ...
    for i in range(len(news_dict)):
        if i not in news_user_click_dict.keys():
            news_user_click_dict[i] = [1] * 1000
    ...
    index = 0
    for key, value in news_user_click_dict.items():
        if len(value) <= margin and index < news_cold_size:
    ...
    for type in news_indictor:
        if type == 'c':
            ...
            news_type.append(1)
            warm_news_num += 1
    np.save('../Data_MIND/metadata/news_type', np.array(news_type))
...
def cal_news_popularity(user_clicked_df, user_pnewsid_nnews_id_df, new_dict):
    ...
    np.save('../Data_MIND/metadata/news_popularity', np.array(pop_list))
...
def get_index_clicked_newsid_cand_newsid( user_clicked_newsid_list, total_newsid_list):
    ...
    # print(news_index_df)
    news_index_df.to_csv('../Data_MIND/df/news_index_df.csv',index= False)
    return news_dict, total_newsid_list
...
def get_test_news_user(news_dict, user_pnewsid_nnews_id_df):
    ...
    np.save('../Data_MIND/test/test_user_index.npy', np.array(userindex))
    np.save('../Data_MIND/test/test_candidate_newsindex.npy', np.array(newsindex_list))
    np.save('../Data_MIND/test/test_label.npy', np.array(label_list))
    np.save('../Data_MIND/test/test_bound.npy', np.array(Bound))
...
def negtivate_sample(user_pnewsid_nnews_id_df, news_dict):
    ...
    newsindex_label_df.to_csv('../Data_MIND/df/newsindex_label_df.csv')

    ## 保存用户index列表
    user_index = np.array(user_index)
    # print('-----------user_index.npy--------------')
    # print(user_index)
    np.save('../Data_MIND/metadata/user_index.npy', user_index)

    ## 保存候选新闻index列表
    candidate_newsindex = np.array(candidate_newsindex_list)
    # print('-----------candidate_newsindex.npy--------------')
    # print(candidate_newsindex)
    np.save('../Data_MIND/metadata/candidate_newsindex.npy', candidate_newsindex)

    ## 保存label列表
    label = np.array(label_list)
    # print('-----------label.npy--------------')
    # print(label)
    np.save('../Data_MIND/metadata/label.npy', label)
...
def construct_clicked_newsindex_cand_newsindex( user_clicked_df,  user_clicked_newsid_list, news_dict, maxlen_clicked ):
    ...
    # print(user_clicked_newsindex)
    np.save('../Data_MIND/metadata/user_clicked_newsindex', user_clicked_newsindex)

    user_clicked_df['user_clicked_newsindex'] = user_clicked_newsindex_list
    user_clicked_df.to_csv('../Data_MIND/df/user_clicked_df.csv',index= False)
...
def get_news_embedding (news_title):
    ...
    np.save('../Data_MIND/metadata/news_title_embedding.npy', np.array(news_title_embedding))
...
def get_title_word_index (word_total_list_t, news_Info_df_select):
    ...
    word_index_df.to_csv('../Data_MIND/df/word_index_df.csv', index= False)
    ...
    np.save('../Data_MIND/metadata/news_title_length.npy', news_title_length)
    ## 求最大长度
    max_len = 0
    for i in range(len(title_word_index)):
    ...
    np.save('../Data_MIND/metadata/news_title_word_index.npy', title_word_index)
...
def merge_title_and_abstract_list(entity_information_t ,entity_information_a, news_Info_df_select):
    ...
    entity_index_df.to_csv('../Data_MIND/df/entity_index_df.csv')
...
def map_entity_information(entity_id_list, news_Info_df_select, max_len):
    ...
    np.save('../Data_MIND/metadata/news_entity_index.npy', entity_index)
...
def extract_entity_vector(entity_index_df, entity_embedding_df):
    ...
    np.save('../Data_MIND/metadata/news_entity_embedding.npy', entity_embedding_index)
...
def get_category_index(news_Info_df_select):
    ...
    np.save('../Data_MIND/metadata/news_category_index.npy', np.array(category_index_list))
    ...
    category_index_df.to_csv('../Data_MIND/df/category_index_df.csv', index=False)
...
def get_subcategory_index(news_Info_df_select):
    ...
    np.save('../Data_MIND/metadata/news_subcategory_index.npy', np.array(subcategory_index_list))
    ...
    subcategory_index_df.to_csv('../Data_MIND/df/subcategory_index_df.csv', index=False)
    print('子主题个数{}'.format(subcategory_index_df.shape[0]))
    news_Info_df_select.to_csv('../Data_MIND/df/news_Info_df.csv')
...
if __name__ == "__main__":
    ...
    ## 获取用户点击新闻id和候选新闻id映射
    news_dict, total_newsid_list = get_index_clicked_newsid_cand_newsid(user_clicked_newsid_list, total_newsid_list)
    ## split冷热用户
    cal_user_clicked_news_ratio(user_dict, user_clicked_newsid_list, news_dict)
    ## split冷热新闻
    news_index_df = cal_news_clicked_user_ratio(user_clicked_df, total_newsid_list, news_dict)
    # ## 获取新闻流行度
