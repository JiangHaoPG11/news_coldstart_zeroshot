...
class Trainer():
    def __init__(self, args, model, optimizer_all, optimizer_base, optimizer_Zeroshot_news, optimizer_Zeroshot_user, data):
        self.args = args
        self.model = model
        self.optimizer_all = optimizer_all
    ...
    def optimize_model(self, rec_score, rec_att_score, label):
    ...
    def optimize_all_model(self, rec_score, rec_att_score, loss_zeroshot_news, loss_zeroshot_user, label):
        ...
        loss = rec_att_loss + rec_loss + loss_zeroshot_news + loss_zeroshot_user
        loss.backward(retain_graph = True)
        #for name, param in self.model_recommender.named_parameters():
        #    print('%14s : %s' % (name, param.grad))
        self.optimizer_all.step()
        return rec_att_loss, rec_loss
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_att_score, rec_score, loss_zeroshot_news, loss_zeroshot_user, La, Lc, Ld = self.model(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
            ...
            # rec_att_loss, rec_loss = self.optimize_model(rec_score, rec_att_score,  label.to(self.device))
            rec_att_loss, rec_loss = self.optimize_all_model(rec_score, rec_att_score,  loss_zeroshot_news, loss_zeroshot_user,  label.to(self.device))
            ...
            rec_att_loss_list.append(rec_att_loss.cpu().item())
            ...
            user_zeroshot_La_loss_list.append(La.cpu().item())
            user_zeroshot_Lc_loss_list.append(Lc.cpu().item())
            user_zeroshot_Ld_loss_list.append(Ld.cpu().item())
            auc_list.append(rec_auc)
            att_auc_list.append(rec_att_auc)
            pbar.update(self.args.batch_size)

        pbar.close()
        return mean(rec_all_loss_list), mean(rec_att_loss_list), mean(news_zeroshot_all_loss_list), mean(user_zeroshot_all_loss_list), \
               mean(user_zeroshot_La_loss_list), mean(user_zeroshot_Lc_loss_list), mean(user_zeroshot_Ld_loss_list), mean(auc_list), mean(att_auc_list)
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_NRMS_ATT = self.args.checkpoint_dir + ('checkpoint-Zeroshot_NRMS_ATT-epoch{}.pth'.format(epoch))
        torch.save(state_model, filename_NRMS_ATT)

    def train(self):
        for epoch in range(1, self.args.epoch+1):
            rec_loss, rec_att_loss, zeroshot_loss_news, zeroshot_loss_user, La, Lc, Ld, rec_auc, rec_att_auc = self._train_epoch(epoch)
            print("epoch：{}----rec loss：{}---rec att loss：{}--zeroshot_news_loss：{}---zeroshot_user_loss：{}----la：{}--lc：{}--ld：{}---rec auc：{}--rec att auc：{}------".
                  format(epoch, str(rec_loss), str(rec_att_loss), str(zeroshot_loss_news), str(zeroshot_loss_user),
                         str(La), str(Lc), str(Ld), str(rec_auc), str(rec_att_auc)))
    ...
    def test(self):
        ...
        predict_df.to_csv('Zeroshot_NRMS_ATT_predict.csv', index = False)
