...
class Zeroshot_NRMS_ATT(torch.nn.Module):
    def __init__(self, args, entity_embedding, relation_embedding, news_entity_dict, entity_adj, relation_adj,
        ...
        super(Zeroshot_NRMS_ATT, self).__init__()
        self.args = args
        self.device = device
    ...
    def forward(self, candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index):
        ...
        att_score = torch.sum(news_att_rep * user_att_rep.view(self.args.batch_size, 1, -1), dim=-1).view(self.args.batch_size, -1)
