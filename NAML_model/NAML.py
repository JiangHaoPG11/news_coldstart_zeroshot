...
class user_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding, category_index, subcategory_index):
        news_rep = self.news_encoder(word_embedding, category_index, subcategory_index).unsqueeze(0)
...
class NAML(torch.nn.Module):
    ...
    def get_user_news_rep(self, candidate_news_index, user_clicked_news_index):
        ...
        for i in range(self.args.batch_size):
            ...
            # 用户表征
            user_rep_one = self.user_encoder(clicked_news_word_embedding_one, clicked_news_category_index,
                                             clicked_news_subcategory_index).unsqueeze(0)
            if i == 0:
