...
class Trainer():
    def __init__(self, args, exp1_model, optimizer_exp1, data):
        self.args = args
        self.exp1_model = exp1_model
        self.optimizer_exp1 = optimizer_exp1
        self.save_period = 1
        self.vaild_period = 1
    ...
    def _train_epoch(self, epoch):
        ...
        for data in self.train_dataloader:
            ...
            rec_score, d_loss, g_loss = self.exp1_model(candidate_newsindex, user_clicked_newsindex)
            rec_loss = self.optimize_model(rec_score, d_loss, g_loss, label.to(self.device))
    ...
    def _vaild_epoch(self):
        ...
        with no_grad():
            ...
            for data in self.vaild_dataloader:
                candidate_newsindex, user_index, user_clicked_newsindex, label, user_type_index, news_type_index = data
                rec_score = self.exp1_model.test(candidate_newsindex, user_clicked_newsindex)
                rec_auc = self.cal_auc(label, rec_score)
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_exp1 = self.args.checkpoint_dir + ('checkpoint-exp1-epoch{}.pth'.format(epoch))
    ...
    def test(self):
        ...
        predict_df.to_csv('GAN_exp1_predict.csv', index = False)
