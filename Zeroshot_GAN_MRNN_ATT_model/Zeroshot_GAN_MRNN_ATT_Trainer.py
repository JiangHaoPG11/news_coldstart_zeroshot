...
class Trainer():
    ...
    def optimize_model(self, rec_score, news_gan_loss, user_gan_loss, label):
        rec_loss = self.criterion(rec_score, torch.argmax(label, dim=1).to(self.device))
        loss = rec_loss + (news_gan_loss + user_gan_loss) * 0.1
    ...
    def _train_epoch(self, epoch):
        ...
        news_gan_all_loss_list = []
        user_gan_all_loss_list = []
        ...
        for data in self.train_dataloader:
            ...
            rec_score, news_gan_loss, user_gan_loss, loss_zeroshot_news, loss_zeroshot_user = self.model(candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index )
            rec_loss = self.optimize_model(rec_score, news_gan_loss, user_gan_loss, label.to(self.device))
            ...
            # news_zeroshot_all_loss_list.append(loss_zeroshot_news)
            # user_zeroshot_all_loss_list.append(loss_zeroshot_user)


            rec_auc = self.cal_auc(label, rec_score)
            rec_all_loss_list.append(torch.mean(rec_loss).cpu().item())

            news_gan_all_loss_list.append(torch.mean(news_gan_loss).cpu().item())
            user_gan_all_loss_list.append(torch.mean(user_gan_loss).cpu().item())
        ...
        return mean(rec_all_loss_list), mean(news_zeroshot_all_loss_list), mean(user_zeroshot_all_loss_list), \
               mean(news_gan_all_loss_list), mean(user_gan_all_loss_list), mean(auc_list)
    ...
    def _save_checkpoint(self, epoch):
        ...
        filename_GAN_MRNN_ATT = self.args.checkpoint_dir + ('checkpoint-Zeroshot_GAN_MRNN_ATT-epoch{}.pth'.format(epoch))
        torch.save(state_model, filename_GAN_MRNN_ATT)

    def train(self):
        for epoch in range(1, self.args.epoch+1):
            rec_loss, zeroshot_loss_news, zeroshot_loss_user, gan_loss_news, gan_loss_user, rec_auc = self._train_epoch(epoch)
            print("epoch：{}----recommend loss：{}-----zeroshot_news_loss：{}---zeroshot_user_loss：{}----gan_news_loss：{}---gan_user_loss：{}--rec auc：{}-------".
                   format(epoch, str(rec_loss), str(zeroshot_loss_news), str(zeroshot_loss_user), 
                   str(gan_loss_news), str(gan_loss_user), str(rec_auc)))
    ...
    def test(self):
        ...
        with no_grad():
            ...
            pbar.close()
        # 存储预测结果
        folder_path = '../predict/GAN_MRNN_ATT/'
        ...
        predict_df.to_csv('Zeroshot_GAN_MRNN_ATT_predict.csv', index = False)
