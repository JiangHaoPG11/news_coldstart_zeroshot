...
class zeroshot_news_simple_tower(torch.nn.Module):
    ...
    def forward(self, news_content_rep, news_att_rep, news_feature_list, news_type_index):
        ...
        # output = torch.where(news_type_index.unsqueeze(-1) == 0, feature_byatt, news_content_rep)

        # output = torch.where(torch.flatten(news_type_index, 0, 1).unsqueeze(1) == 0,
        #                      torch.flatten(att_byatt, 0, 1),
        #                      torch.flatten(news_att_rep, 0, 1)).view(self.args.batch_size, self.args.sample_size, -1)
        return loss_zeroshot, feature_byatt
...
class zeroshot_user_simple_tower(torch.nn.Module):
    ...
    def forward(self, user_beh_rep, user_att_rep, user_type_index):
        ...
        return loss_zeroshot, beh_byatt, La, Lc, Ld
...
class zeroshot_user_contrastive_tower(torch.nn.Module):
    ...
    def forward(self, user_beh_rep, user_att_rep, user_type_index):
        ...
        user_att_rep = user_att_transfer_rep + user_beh_rep
        return loss_zeroshot, user_att_transfer_rep, torch.mean(Lp.squeeze()),torch.mean(Ln1.squeeze() + Ln2.squeeze())
...
class zeroshot_news_contrastive_tower(torch.nn.Module):
    ...
    def forward(self, news_content_rep, news_att_rep, news_feature_list, news_type_index):
        ...
        contrast_loss = nn.CrossEntropyLoss()
        loss_zeroshot = contrast_loss(torch.cat([Lp.unsqueeze(1), Ln1.unsqueeze(1),
                                        Ln2.unsqueeze(1)], dim=1), torch.zeros_like(Lp,dtype=torch.long))   
...
class zeroshot_user_gaz_tower(torch.nn.Module):
    ...
    def discriminator(self, embedding):
        ...
        embedding = self.relu(self.mlp1(embedding))
        embedding = F.dropout(embedding, p=self.dropout_prob, training=self.training)
    ...
    def forward(self, user_beh_rep, user_att_rep, user_type_index):
        ...
        # cal d loss
        score1 = self.discriminator(torch.cat([user_beh_rep_old, user_att_rep_old], dim = -1))
        score2 = self.discriminator(torch.cat([user_att_transfer_rep, user_att_rep_old], dim = -1))
        ...
        g_score = self.discriminator(torch.cat([user_generate_rep, user_generate_rep], dim =-1))
...
class zeroshot_news_gaz_tower(torch.nn.Module):
    ...
    def news_att_transfer(self, x):
        x1 = self.relu(self.news_att_transfer_layer1(x))
    ...
    def forward(self, news_content_rep, news_att_rep, news_feature_list, news_type_index):
        ...
        # cal d loss
        score1 = self.discriminator(torch.cat([news_content_rep_old, news_att_rep_old], dim = -1))
        score2 = self.discriminator(torch.cat([news_att_transfer_rep, news_att_rep_old], dim = -1))
        d_loss = self.loss1(torch.cat([score1.unsqueeze(1), score2.unsqueeze(1)], dim = 1), torch.zeros_like(score1, dtype=torch.long) )
        ...
        g_score = self.discriminator(torch.cat([news_generate_rep, news_generate_rep], dim = -1))
        g_loss = self.loss2(torch.flatten(g_score, -2, -1), torch.zeros_like(torch.flatten(g_score, -2, -1)) )
