...
def metadata_generator(args, path):
    def split_data(news_data_mode, user_data_mode, candidate_newsindex, user_index, label, news_type, user_type):
        if user_data_mode == 2 and news_data_mode == 2:
            ...
            elif news_data_mode == 2 and user_data_mode != 2:
    ...
    # 嵌入数据
    news_title_embedding = np.load(path + '/news_cold_start_code/Data_MIND/metadata/news_title_embedding.npy')
    # 新闻类型
    news_type = np.load(path + '/news_cold_start_code/Data_MIND/metadata/news_type.npy')
    # 新闻实体index
    news_entity_index = np.load(path + '/news_cold_start_code/Data_MIND/metadata/news_entity_index.npy')
    # 用户点击新闻
    user_clicked_newsindex = np.load(path + '/news_cold_start_code/Data_MIND/metadata/user_clicked_newsindex.npy')
    # 用户类型类型
    user_type = np.load(path + '/news_cold_start_code/Data_MIND/metadata/user_type.npy')
    # 训练集
    candidate_newsindex = np.load(path + '/news_cold_start_code/Data_MIND/metadata/candidate_newsindex.npy')
    user_index= np.load(path + '/news_cold_start_code/Data_MIND/metadata/user_index.npy')
    label = np.load(path + '/news_cold_start_code/Data_MIND/metadata/label.npy')

    # 测试集
    candidate_newsindex_test = np.load(path + '/news_cold_start_code/Data_MIND/test/test_candidate_newsindex.npy')
    user_index_test = np.load(path + '/news_cold_start_code/Data_MIND/test/test_user_index.npy')
    label_test = np.load(path + '/news_cold_start_code/Data_MIND/test/test_label.npy')
    Bound_test = np.load(path + '/news_cold_start_code/Data_MIND/test/test_bound.npy')
...
def load_entity_relation_dict(path):
    ...
    entityid2index = pd.read_csv( path + '/news_cold_start_code/Data_MIND/KG/entityid2index.csv')
    relationid2index = pd.read_csv(path + '/news_cold_start_code/Data_MIND/KG/relationid2index.csv')
    entity_dict = {}
    relation_dict = {}
...
def build_KG_network(path, news_entity_dict):
    ...
    graph = pd.read_csv(path + '/news_cold_start_code/Data_MIND/KG/graph_index.csv')
...
def build_entity_relation_embedding(path):
    ...
    TransE_entity_embedding = np.load(path+'/news_cold_start_code/Data_MIND/KG/TransE_entity_embedding.npy')
    TransE_relation_embedding = np.load(path + '/news_cold_start_code/Data_MIND/KG/TransE_relation_embedding.npy')
...
def build_word_embedding(path):
    ...
    word_embedding = np.load(path+'/news_cold_start_code/Data_MIND/metadata/news_title_word_embedding.npy')
