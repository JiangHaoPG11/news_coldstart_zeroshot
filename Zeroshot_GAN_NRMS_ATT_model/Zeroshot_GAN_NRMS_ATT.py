...
class user_encoder(torch.nn.Module):
    ...
    def forward(self, word_embedding):
        ...
        news_rep = self.multiheadatt(news_rep.unsqueeze(0))
        news_rep = F.dropout(news_rep, p=self.dropout_prob, training=self.training)
        user_rep = torch.tanh(self.user_attention(news_rep))
        # user_rep = self.norm(user_rep)
        return user_rep

class Zeroshot_GAN_NRMS_ATT(torch.nn.Module):
    def __init__(self, args, entity_embedding, relation_embedding, news_entity_dict, entity_adj, relation_adj,
        ...
        super(Zeroshot_GAN_NRMS_ATT, self).__init__()
        self.args = args
        self.device = device
    ...
    def forward(self, candidate_newsindex, user_index, user_clicked_newsindex, user_type_index, news_type_index):
        ...
        print(user_rep_gan.shape)
        print(news_rep_gan.shape)
        print(newsatt_rep_gan.shape)
        print(useratt_rep_gan.shape)
        ...
        user = torch.where(user_type_index.to(self.device).unsqueeze(1) == 0, useratt_rep.squeeze(), user_rep.squeeze()).view(self.args.batch_size, 1, -1)
